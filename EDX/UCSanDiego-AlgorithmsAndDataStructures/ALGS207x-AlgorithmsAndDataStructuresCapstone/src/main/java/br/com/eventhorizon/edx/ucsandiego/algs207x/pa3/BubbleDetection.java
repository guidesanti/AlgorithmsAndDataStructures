...
public class BubbleDetection implements PA {
  ...
  private static void buildDeBruijnGraph() {
    ...
    Map<String, List<Integer>> outEdges = new HashMap<>();
    Map<String, List<Integer>> inEdges = new HashMap<>();
    ...
    for (String read : reads) {
      for (int i = 0; i <= readLength - k; i++) {
        ...
        if(!indices.containsKey(fromLabel)){
          ...
          outEdges.put(fromLabel, new ArrayList<>());
          inEdges.put(fromLabel, new ArrayList<>());
          index++;
        }
        ...
        if(!indices.containsKey(toLabel)){
          ...
          outEdges.put(toLabel, new ArrayList<>());
          inEdges.put(toLabel, new ArrayList<>());
          index++;
        }
        int toIndex = indices.get(toLabel);

        if (isOverlap(fromLabel, toLabel)) {
          outEdges.get(fromLabel).add(indices.get(toLabel));
          inEdges.get(toLabel).add(indices.get(fromLabel));
        }
    ...
    indices.forEach((l, i) -> graph[i] = new Vertex(i, l, outEdges.get(l), inEdges.get(l)));
    edges.forEach(edge -> {
  ...
  private static class Vertex {
    ...
    final List<Integer> outEdges;

    final List<Integer> inEdges;

    final List<Edge> outEdges2;

    final List<Edge> inEdges2;

    Vertex(int index, String label, List<Integer> outEdges, List<Integer> inEdges) {
      this.index = index;
      this.label = label;
