...
public class FindEulerianPathInGraphTest extends PATest {

  private static final Logger LOGGER = Logger.getLogger(FindEulerianPathInGraphTest.class.getName());

  private static final String SIMPLE_DATA_SET = "/test-dataset/pa1/find-eulerian-path-in-graph.csv";

  public FindEulerianPathInGraphTest() {
    super(new FindEulerianPathInGraph());
  }
  ...
  protected String generateInput(PATestType type) {
    ...
    String label = adjacencies.keySet().stream().findAny().get();
    List<String> adj = adjacencies.get(label);
    adj.remove(0);
    if (adj.isEmpty()) {
      adjacencies.remove(label);
    }
